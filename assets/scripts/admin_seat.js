...
function showSuggestions()
{
    ...
    let suggestions = busData.filter(({kereta_no}) => {
        return kereta_no.match(regex);
    }).map(({kereta_no}) => {
        const kereta_num = kereta_no.replace(regex, `<span class="hl">${this.value.toUpperCase()}</span>`);;
        return `<li>${kereta_num}</li>`;
    }).join("");
